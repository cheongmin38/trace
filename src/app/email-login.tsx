...
export default function EmailLogin() {
  ...
  const submit = async () => {
    const e = !pattern.test(email.trim())
      ? "이메일 주소를 확인해주세요."
      : password.length < 8
        ? "비밀번호는 8자 이상 입력해주세요."
        : null;
    ...
    try {
      ...
      console.error("Trace email login failed", cause);
    }
  };
  ...
  return (
    ...
    >
      ...
      >
        <Stack.Screen options={{ title: "이메일 로그인" }} />
        <Pressable onPress={() => router.back()}>
          <Ionicons name="arrow-back" size={24} color={colors.text} />
        </Pressable>
        <View style={styles.header}>
          <ThemedText variant="largeTitle">이메일로 계속하기</ThemedText>
          <ThemedText variant="subhead" style={{ color: colors.secondaryText }}>
            Trace 계정으로 기록을 이어가세요.
          </ThemedText>
        </View>
        <View style={styles.form}>
          ...
          <View style={[styles.input, { backgroundColor: colors.surface }]}>
            ...
            <TextInput
              ...
              style={[styles.field, typography.body, { color: colors.text }]}
            />
            <PressableScale onPress={() => setVisible((v) => !v)}>
              <Ionicons
          ...
          <PressableScale onPress={() => router.push("/forgot-password")}>
            <ThemedText
              variant="caption"
