...
import { generateAskTraceAnswer } from '@/services/ask-trace-answer-service';
...
export default function AskTraceScreen(){
 ...
 const submit=async()=>{if(!q.trim())return;setLoading(true);const r=await searchAskTraceQuestion(q,scopedUserId,{getPlaces:async()=>places,getVisits:async()=>visits,getMemories:async()=>memories});setResult(r);setAnswer((await generateAskTraceAnswer(q,r)).text);setLoading(false);};
