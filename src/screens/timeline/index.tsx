  ...
  return <View style={[styles.root, { backgroundColor: colors.background }]}>
    {loading ? <View style={styles.skeleton}><SkeletonCard /><SkeletonCard /></View> : <SectionList
      ...
      ListHeaderComponent={<View style={styles.header}>
        ...
        <PressableScale onPress={() => setMonthOpen(true)} style={[styles.monthSelector, { backgroundColor: colors.surfaceGlass, borderColor: colors.border, boxShadow: shadow.soft }]}><ThemedText variant="headline">{monthTitle(month)}</ThemedText><Ionicons name="chevron-down" size={18} color={colors.secondaryText} /></PressableScale>
        <View style={styles.chips}>{contentFilters.map((item) => <PressableScale key={item.key} onPress={() => setContentFilter(item.key)} style={[styles.chip, { backgroundColor: contentFilter === item.key ? colors.accent : colors.surfaceGlass, borderColor: colors.border }]}><ThemedText variant="caption" style={{ color: contentFilter === item.key ? colors.onAccent : colors.secondaryText }}>{item.label}</ThemedText></PressableScale>)}</View>
      </View>}
...
const styles = StyleSheet.create({ root: { flex: 1 }, list: { paddingHorizontal: spacing.ml, paddingBottom: 124 }, header: { paddingTop: spacing.lg, paddingBottom: spacing.md, gap: spacing.md }, skeleton: { padding: spacing.ml, gap: spacing.xl }, monthSelector: { minHeight: 52, borderRadius: radius.md, borderWidth: StyleSheet.hairlineWidth, paddingHorizontal: spacing.md, flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between' }, chips: { flexDirection: 'row', gap: spacing.xs }, chip: { paddingHorizontal: spacing.md, paddingVertical: spacing.sm, borderRadius: radius.full, borderWidth: StyleSheet.hairlineWidth }, dayHeader: { marginTop: spacing.lg, marginBottom: spacing.sm, flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center' }, indicator: { width: 6, height: 6, borderRadius: radius.full }, row: { flexDirection: 'row', minHeight: 106 }, timeColumn: { width: 54, alignItems: 'flex-end', paddingRight: spacing.sm }, time: { paddingTop: spacing.md }, line: { width: 1, flex: 1, marginTop: spacing.xs }, dot: { width: 9, height: 9, borderRadius: radius.full, marginTop: spacing.lg, marginRight: spacing.sm }, rowCard: { flex: 1, borderRadius: radius.md, borderWidth: StyleSheet.hairlineWidth, padding: spacing.sm, marginBottom: spacing.sm, flexDirection: 'row', gap: spacing.sm }, rowCopy: { flex: 1, gap: spacing.xs, justifyContent: 'center' }, placeLine: { flexDirection: 'row', alignItems: 'center', gap: spacing.xs }, placeName: { flex: 1 }, badge: { borderRadius: radius.full, paddingHorizontal: spacing.xs, paddingVertical: 2 }, thumbs: { flexDirection: 'row', alignItems: 'center', gap: spacing.xs, marginTop: spacing.xs }, thumbnail: { width: 72, height: 72, borderRadius: radius.sm }, scrim: { flex: 1, justifyContent: 'flex-end' }, sheet: { paddingHorizontal: spacing.ml, paddingTop: spacing.sm, paddingBottom: spacing.xl, borderTopLeftRadius: radius.lg, borderTopRightRadius: radius.lg, gap: spacing.sm }, grabber: { width: 38, height: 4, borderRadius: radius.full, alignSelf: 'center', marginBottom: spacing.sm }, monthRow: { minHeight: 52, flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between' }, monthRight: { flexDirection: 'row', alignItems: 'center', gap: spacing.sm }, calendarDot: { width: 6, height: 6, borderRadius: radius.full } });
