...
const fail = (res: ApiResponse, status: number, message: string) => res.status(status).json({ error: message });

export default async function handler(req: ApiRequest, res: ApiResponse) {
  if (req.method !== 'POST') return fail(res, 405, 'Method not allowed');
  const apiKey = process.env.OPENAI_API_KEY;
  if (!apiKey) return fail(res, 503, 'AI service is not configured');
  const body = req.body as { question?: unknown; result?: unknown } | undefined;
